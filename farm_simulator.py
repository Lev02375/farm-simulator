...
class FarmGame:
    ...
    def setup_ui(self):
        ...
        for crop_id, crop in self.crops.items():
            ...
            if locked: info = info + ' | LOCK Lv.' + str(crop.unlock_level)
        ...
        help_text += 'WATER:' + chr(10)
        help_text += '- Buy Watering Can first!' + chr(10)
        help_text += '- Each water costs 5 units' + chr(10) + chr(10)
        help_text += '18 CROPS (1-12 min grow time)' + chr(10)
        help_text += '- Unlock by leveling up' + chr(10) + chr(10)
        ...
        tk.Button(cheat_frame, text='EXECUTE', font=('Arial', 9, 'bold'), bg='#E74C3C', fg='white', command=self.execute_cheat).pack(side=tk.RIGHT, padx=8, pady=6)
        self.log('Welcome to Farm Simulator!')
        self.log('Plants take MINUTES to grow!')
    ...
    def update_cell(self, row, col):
        ...
        if row >= self.grid_size or col >= self.grid_size:
            ...
            btn.grid()
        if cell.stage == 'empty':
            btn.config(text='[SOIL]', bg='#6D4C41', fg='#D7CCC8',
    ...
    def buy_watering_can(self):
        if not self.has_watering_can:
            if self.money >= 80:
                ...
                self.log('Bought Watering Can! RMB to water.'); self.update_stats()
            else: messagebox.showwarning('No Money', 'Need $80')
        elif self.watering_can_level == 1:
            if self.money >= 200:
                self.money -= 200; self.watering_can_level = 2
                self.log('Advanced Watering Can! 3x3 area!'); self.update_stats()
            else: messagebox.showwarning('No Money', 'Need $200')
    def buy_auto_water(self):
        if not self.auto_water and self.money >= 1200:
            self.money -= 1200; self.auto_water = True; self.log('Auto-Water ON!'); self.update_stats()
    ...
    def expand_farm(self):
        if self.grid_size < 6:
            ...
            if self.money >= cost:
                self.money -= cost; self.grid_size += 1; self.log('Farm ' + str(self.grid_size) + 'x' + str(self.grid_size) + '!'); self.update_stats()
            for i in range(self.grid_size):
                for j in range(self.grid_size):
                    self.cell_buttons[i][j].grid(); self.update_cell(i, j)
    def buy_greenhouse(self):
        if not self.greenhouse and self.money >= 2000:
            self.money -= 2000; self.greenhouse = True; self.log('Greenhouse ON!'); self.update_stats()
    def buy_auto_harvest(self):
        if not self.auto_harvest and self.money >= 3000:
            self.money -= 3000; self.auto_harvest = True; self.log('Auto-Harvest ON!'); self.update_stats()
    def buy_pest_control(self):
        if self.money >= 800:
            self.money -= 800; self.log('Pest Control ON!'); self.update_stats()
    def buy_marketing(self):
        if self.price_bonus < 1.5 and self.money >= 1000:
            self.money -= 1000; self.price_bonus += 0.25; self.log('Price bonus +' + str(int((self.price_bonus-1)*100)) + '%!'); self.update_stats()
    def buy_water_tank(self):
        if self.money >= 300:
            self.money -= 300; self.max_water += 25; self.water = min(self.max_water, self.water + 25); self.log('Water max: ' + str(self.max_water)); self.update_stats()
    def buy_rain_collector(self):
        if not self.rain_collector and self.money >= 1500:
            self.money -= 1500; self.rain_collector = True; self.log('Rain Collector ON!'); self.update_stats()
    def level_up(self):
